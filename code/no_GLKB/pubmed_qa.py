...
parser = argparse.ArgumentParser(description="The testing code for PubMedQA (1000 questions) with no GLKB or external resources.")
...
PROMPT = '''
Task: answer the following question, using "yes", "no", or "maybe".
...
The possibility of "maybe" in ground truth is LESS THAN 5%.
...
def check_format(response: str) -> tuple[bool, str]:
    try:
        ...
        assert (response['answer'] in ['yes', 'maybe', 'no'])
        return (True, '')
    except:
...
OUTPUT_PATH = f'./outputs/pubmedqa_{llm}_{time.time()}.json'
